...
def scale_galaxy() -> tuple[int, float]:
    """Scale galaxy dimensions based on radio bubble size (scale)."""
    disc_raduis_scaled = round(DISC_RADIUS / SCALE)
    bubble_vol = 4/3 * math.pi * (SCALE / 2)**3
    disc_vol_scaled = DISC_VOL / bubble_vol
    return disc_raduis_scaled, disc_vol_scaled


def detect_prob(disc_vol_scaled: float) -> float:
...
def random_polar_coordinates(disc_radius_scaled: int) -> tuple[int, int]:
...
def spirals(b: float, r: int, rot_fac: float, fuz_fac: float, arm: int) -> None:
...
def star_haze(disc_radius_scaled: int, density: int):
